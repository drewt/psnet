        ...
        const char *value)
{
	...
	if (!strcmp(name, "listen-port")) {
		if (!(val = atoi(value)))
			printf("%s: error: listen-port must be "
					"a positive integer\n",
					(char*) user);
		else
		...
		if (!(val = atoi(value)))
			printf("%s: error: max-threads must be "
					"a positive integer\n",
					(char*) user);
		else
...
void parse_opts(int argc, char *argv[], struct settings *dst)
{
	...
	for(;;) {
		...
		c = getopt_long(argc, argv, "t:l:", long_options,
				&options_index);
		...
		case 't':
			...
			if (dst->max_threads < 1
					|| (endptr && *endptr != '\0')) {
				puts("error: --threads argument "
